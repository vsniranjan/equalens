...
const SAFE_ATTRIBUTES = [
  ...
  "multiple", "name", "placeholder", "rel", "required", "role", "rowspan", "scope", "selected", "src", "step",
...
export interface ElementSnapshot {
  target: HTMLElement;
  originalHtml: string;
  originalAttributes: ReadonlyArray<readonly [string, string]>;
  clone: HTMLElement;
  originalNodes: Node[];
}
...
export function captureElementSnapshot(target: HTMLElement): ElementSnapshot {
  return {
    ...
    originalAttributes: [...target.attributes].map(({ name, value }) => [name, value] as const),
    clone: cloneWithComputedStyles(target),
    originalNodes: [...target.childNodes],
  };
}

export function applySanitizedRedesign(snapshot: ElementSnapshot, sanitizedHtml: string): void {
  const replacement = replacementInnerHtml(snapshot.target, sanitizedHtml);
  snapshot.target.innerHTML = replacement;
...
export function restoreElementSnapshot(snapshot: ElementSnapshot): void {
  for (const attribute of [...snapshot.target.attributes]) snapshot.target.removeAttribute(attribute.name);
  for (const [name, value] of snapshot.originalAttributes) snapshot.target.setAttribute(name, value);
  // Reattach the actual nodes so page-owned listeners and live form state survive undo.
  snapshot.target.replaceChildren(...snapshot.originalNodes);
}

export function preserveRedesignedFormState(snapshot: ElementSnapshot): void {
  const originals = [...snapshot.clone.querySelectorAll<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>("input, textarea, select")];
  for (const target of snapshot.target.querySelectorAll<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>("input, textarea, select")) {
...
function replacementInnerHtml(target: HTMLElement, sanitizedHtml: string): string {
  const template = target.ownerDocument.createElement("template");
  template.innerHTML = sanitizedHtml;
  const significantNodes = [...template.content.childNodes]
  ...
  const onlyElement = significantNodes.length === 1 && significantNodes[0]?.nodeType === Node.ELEMENT_NODE
    ...
    : null;
  return onlyElement?.tagName === target.tagName ? onlyElement.innerHTML : sanitizedHtml;
}
...
function removeSnapshotIds(root: HTMLElement): void {
  ...
  for (const element of [root, ...root.querySelectorAll<HTMLElement>("[id]")]) {
    ...
    element.id = `eqx-redesign-snapshot-${index}`;
    index += 1;
  }
}
...
function cloneWithComputedStyles(target: HTMLElement): HTMLElement {
  ...
  sources.forEach((source, index) => {
    ...
    for (let propertyIndex = 0; propertyIndex < style.length; propertyIndex += 1) {
      ...
      copy.style.setProperty(property, style.getPropertyValue(property), style.getPropertyPriority(property));
    }
...
function copyFormState(source: HTMLElement, clone: HTMLElement): void {
  if (source.tagName === "INPUT" && clone.tagName === "INPUT") {
    (clone as HTMLInputElement).value = (source as HTMLInputElement).value;
  ...
  if (source.tagName === "SELECT" && clone.tagName === "SELECT") {
    (clone as HTMLSelectElement).selectedIndex = (source as HTMLSelectElement).selectedIndex;
  }
}
