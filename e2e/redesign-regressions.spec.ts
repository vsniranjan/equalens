...
test("redesign-all changes every flagged area and compares every changed section", async ({ context, page }, testInfo) => {
  ...
  const comparison = page.locator(".eqx-redesign-comparison");
  await expect(page.locator(".eqx-redesign-title")).toContainText(demoFindings[0]!.title);
  await expect(comparison).toBeVisible();
  const reviewBox = await comparison.boundingBox();
  expect.soft(reviewBox!.y).toBeGreaterThanOrEqual(12);
  expect.soft(reviewBox!.y + reviewBox!.height).toBeLessThanOrEqual(988);
  expect.soft(await comparison.evaluate((element) => element.scrollTop)).toBe(0);
  const moveHandle = page.locator('[aria-label="Move comparison dialog. Use arrow keys or drag."]');
  const handleBox = await moveHandle.boundingBox();
  await page.mouse.move(handleBox!.x + handleBox!.width / 2, handleBox!.y + handleBox!.height / 2);
  await page.mouse.down();
  await page.mouse.move(4, 4);
  await page.mouse.up();
  const draggedBox = await comparison.boundingBox();
  expect.soft(draggedBox!.x).toBeCloseTo(12, 0);
  expect.soft(draggedBox!.y).toBeCloseTo(12, 0);
  await moveHandle.focus();
  await page.keyboard.press("ArrowRight");
  const keyboardMovedBox = await comparison.boundingBox();
  ...
  await page.locator("#seat-system").scrollIntoViewIfNeeded();
  await expect(page.locator(".eqx-redesign-title")).toContainText(demoFindings[0]!.title);
  await expect(page.locator(".eqx-redesign-comparison")).toHaveCSS("left", "28px");
  await expect(page.locator(".eqx-redesign-comparison")).toHaveCSS("top", "12px");
